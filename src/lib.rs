//! Provides functionality to get `n` items from a `&mut [T]`.
//!
//! This library can provide significant performance increase compared to sorting the whole list
//! when `n` is relatively small.
//!
//! ```text
//! N = 100, LEN = 1_000_000, RANGE = 1_000_000:
//! test max           ... bench:   5,907,759 ns/iter (+/- 191,096)
//! test max_unstable  ... bench:   5,792,875 ns/iter (+/- 106,412)
//! test sort          ... bench:  67,507,274 ns/iter (+/- 958,881)
//! test sort_unstable ... bench:  35,584,123 ns/iter (+/- 1,157,209)
//! ```

#![cfg_attr(not(feature = "use_std"), no_std)]
#![doc(html_root_url = "https://docs.rs/out/0.5.1")]
#![deny(
    bad_style,
    bare_trait_objects,
    missing_debug_implementations,
    missing_docs,
    unused_import_braces,
    unused_qualifications,
    unsafe_code
)]

#[cfg(feature = "use_std")]
use std::cmp::Ordering;
#[cfg(feature = "use_std")]
use std::mem;

#[cfg(not(feature = "use_std"))]
use core::cmp::Ordering;
#[cfg(not(feature = "use_std"))]
use core::mem;

/// Get the `n` largest items.
///
/// # Examples
/// ```
/// let mut v = [-5, 4, 1, -3, 2];
/// let max = out::max(&mut v, 3);
/// assert_eq!(max, [1, 2, 4]);
/// ```
#[inline]
#[cfg(feature = "use_std")]
pub fn max<T: Ord>(v: &mut [T], n: usize) -> &mut [T] {
    fetch(v, n, |a, b| a.cmp(b))
}

/// Get the `n` largest items.
///
/// # Examples
/// ```
/// let mut v = [-5, 4, 1, -3, 2];
/// let max = out::max_unstable(&mut v, 3);
/// assert_eq!(max, [1, 2, 4]);
/// ```
#[inline]
pub fn max_unstable<T: Ord>(v: &mut [T], n: usize) -> &mut [T] {
    fetch_unstable(v, n, |a, b| a.cmp(b))
}

/// Get the `n` largest items decided by a key generated by `f`.
///
/// # Examples
/// ```
/// let mut v = [-5_i32, 4, 1, -3, 2];
/// let max = out::max_by_key(&mut v, 3, |a| a.abs());
/// assert_eq!(max, [-3, 4, -5]);
/// ```
#[inline]
#[cfg(feature = "use_std")]
pub fn max_by_key<T, K: Ord>(v: &mut [T], n: usize, mut f: impl FnMut(&T) -> K) -> &mut [T] {
    fetch(v, n, |a, b| f(a).cmp(&f(b)))
}

/// Get the `n` largest items decided by a key generated by `f`.
///
/// # Examples
/// ```
/// let mut v = [-5_i32, 4, 1, -3, 2];
/// let max = out::max_unstable_by_key(&mut v, 3, |a| a.abs());
/// assert_eq!(max, [-3, 4, -5]);
/// ```
#[inline]
pub fn max_unstable_by_key<T, K: Ord>(
    v: &mut [T],
    n: usize,
    mut f: impl FnMut(&T) -> K,
) -> &mut [T] {
    fetch_unstable(v, n, |a, b| f(a).cmp(&f(b)))
}

/// Get the `n` smallest items.
///
/// # Examples
/// ```
/// let mut v = [-5, 4, 1, -3, 2];
/// let min = out::min(&mut v, 3);
/// assert_eq!(min, [1, -3, -5]);
/// ```
#[inline]
#[cfg(feature = "use_std")]
pub fn min<T: Ord>(v: &mut [T], n: usize) -> &mut [T] {
    fetch(v, n, |a, b| b.cmp(a))
}

/// Get the `n` smallest items.
///
/// # Examples
/// ```
/// let mut v = [-5, 4, 1, -3, 2];
/// let min = out::min_unstable(&mut v, 3);
/// assert_eq!(min, [1, -3, -5]);
/// ```
#[inline]
pub fn min_unstable<T: Ord>(v: &mut [T], n: usize) -> &mut [T] {
    fetch_unstable(v, n, |a, b| b.cmp(a))
}

/// Get the `n` smallest items decided by a key generated by `f`.
///
/// # Examples
/// ```
/// let mut v = [-5_i32, 4, 1, -3, 2];
/// let min = out::min_by_key(&mut v, 3, |a| a.abs());
/// assert_eq!(min, [-3, 2, 1]);
/// ```
#[inline]
#[cfg(feature = "use_std")]
pub fn min_by_key<T, K: Ord>(v: &mut [T], n: usize, mut f: impl FnMut(&T) -> K) -> &mut [T] {
    fetch(v, n, |a, b| f(b).cmp(&f(a)))
}

/// Get the `n` smallest items decided by a key generated by `f`.
///
/// # Examples
/// ```
/// let mut v = [-5_i32, 4, 1, -3, 2];
/// let min = out::min_unstable_by_key(&mut v, 3, |a| a.abs());
/// assert_eq!(min, [-3, 2, 1]);
/// ```
#[inline]
pub fn min_unstable_by_key<T, K: Ord>(
    v: &mut [T],
    n: usize,
    mut f: impl FnMut(&T) -> K,
) -> &mut [T] {
    fetch_unstable(v, n, |a, b| f(b).cmp(&f(a)))
}

#[inline]
#[cfg(feature = "use_std")]
fn fetch<T>(v: &mut [T], n: usize, mut f: impl FnMut(&T, &T) -> Ordering) -> &mut [T] {
    if n == 0 {
        return &mut [];
    }
    let (max, v) = v.split_at_mut(n);
    max.sort_by(&mut f);
    for x in v {
        if f(x, &max[0]) != Ordering::Greater {
            continue;
        } else {
            mem::swap(x, &mut max[0]);
            let mut i = 0;
            while i < n - 1 && f(&max[i], &max[i + 1]) == Ordering::Greater {
                max.swap(i, i + 1);
                i += 1;
            }
        }
    }
    max
}

#[inline]
fn fetch_unstable<T>(v: &mut [T], n: usize, mut f: impl FnMut(&T, &T) -> Ordering) -> &mut [T] {
    if n == 0 {
        return &mut [];
    }
    let (max, v) = v.split_at_mut(n);
    max.sort_unstable_by(&mut f);
    for x in v {
        if f(x, &max[0]) != Ordering::Greater {
            continue;
        } else {
            mem::swap(x, &mut max[0]);
            let mut i = 0;
            while i < n - 1 && f(&max[i], &max[i + 1]) == Ordering::Greater {
                max.swap(i, i + 1);
                i += 1;
            }
        }
    }
    max
}
